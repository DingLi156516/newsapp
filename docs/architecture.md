...
3. **Deduplication** (`lib/rss/dedup.ts`) — checks article URLs against the database in batches of 20. Already-seen URLs are filtered out.
...
This runs three sub-stages in sequence:

#### 2a. Embedding (`lib/ai/embeddings.ts`)
...
- Processes up to 100 articles per run, in batches of 20
- Once embedded, the article is flagged `is_embedded = true` and never re-processed
...
5. A cluster needs **at least 2 articles** from different sources to become a story (a single article isn't a "story" yet)

Only looks at articles from the last **72 hours** to keep clusters current.

#### 2c. Assembly (`lib/ai/story-assembler.ts`)

For each new story (identified by having the placeholder headline "Pending headline generation"), the system runs **5 AI operations in parallel**:
...
|------|-------|-------------|
| **Home feed** | `/` | Filterable story feed with tabs (For You/Trending/Latest/Blindspot/Saved), topic pills, search, advanced filters (bias range, factuality, date, perspective presets) |
| **Story detail** | `/story/[id]` | Hero image, headline, spectrum bar, AI perspective tabs (Common Ground / Left / Right), source list |
| **Sources** | `/sources` | Searchable directory of all 55 outlets with bias tags, factuality dots, ownership labels |
...
| **Settings** | `/settings` | User preferences form (topics, perspective, factuality) |
...
- **SearchFilters** — expandable panel with topic, bias range, factuality, date range, and perspective preset filters
...
lib/api/         — API utilities (16 files)
  query-helpers.ts  — Supabase query builders; functions: queryStories(), queryStoryById(),
                      querySourcesForStory(), querySources()
...
lib/hooks/       — SWR data-fetching hooks + auth hooks + utilities (17 files)
...
**GET /api/stories** — `topic`, `search`, `blindspot`, `biasRange`, `minFactuality`, `datePreset`, `page` (default 1), `limit` (default 20, max 50)
...
| **stories** | 22+ | Clustered stories: AI headline, topic, spectrum, AI summary, blindspot flag |
...
| **user_preferences** | per-user | user_id FK, followed_topics, default_region, default_perspective, factuality_minimum (migration 004) |
| **admin_users** | per-admin | user_id FK (unique), role, created_at (migration 006) |

The **stories** table includes review columns added by migration 006: `review_status` (pending/approved/rejected, default 'pending'), `reviewed_by` (FK to admin_users), `reviewed_at`.
...
> **Region filtering** was removed from the UI. Content-based region classification is planned as a future phase — see F-08 in `PRD_Axiom_News.md`.
...
5. **API** — User opens Axiom, the story appears in the feed with a spectrum bar and source count badge
6. **Detail** — User clicks in, sees NPR and Fox News listed as covering sources, reads how left outlets emphasize environmental impact while right outlets focus on economic costs
