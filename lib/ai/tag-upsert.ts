...
/**
 * Upsert the given entities as this story's tag set.
 *
 * Three-state input contract (mirrors `extractEntities`):
 *  - `null`        — preserve existing story_tags (no-op). Use when the
 *                    extractor could not produce a signal for this story;
 *                    we don't want to wipe tags generated by a prior run.
 *  - `[]`          — clear existing story_tags. Use when the story is
 *                    genuinely empty and any previous tags are stale.
 *  - `[...]`       — replace existing story_tags with the provided set.
 */
