/**
 * useLocalReadingHistory — AsyncStorage-backed reading history for unauthenticated users.
 * Follows the same pattern as use-local-bookmarks.ts.
 */
...
export interface LocalReadingHistoryHook {
 ...
 */
export async function readLocalReadingHistory(): Promise<ReadonlySet<string>> {
  try {
...
/**
 * Clear local reading history after successful merge.
 */
export async function clearLocalReadingHistory(): Promise<void> {
  try {
...
export function useLocalReadingHistory(): LocalReadingHistoryHook {
